...
export const createSubtopic = async ({
  ...
  try {
    ...
    //Always append description, even if empty (or provide default)
    formData.append('description', description || 'Default subtopic description');
    
    

    // Always append contentType
    formData.append('contentType', contentType);

    // Handle content-specific fields
    if (contentType === 'text') {
      // Always append text_content for text type, even if empty
      formData.append('text_content', text_content || '');
    }

    if (contentType === 'video') {
      if (videoName  && videoName.trim()) formData.append('videoName', videoName);
      if (videoUrl  && videoUrl.trim()) formData.append('videoUrl', videoUrl);
    ...
    const response = await apiConnector('POST', CREATE_SUBTOPIC_API, formData);
